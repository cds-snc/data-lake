...
#
# Glue ETL data anomalies
#
resource "aws_cloudwatch_log_metric_filter" "glue_etl_data_pythonshell_anomaly" {
  name           = "glue-etl-data-pythonshell-anomaly"
  pattern        = local.glue_etl_metric_filter_anomaly_pattern
  log_group_name = "${var.glue_etl_pythonshell_log_group_name}/error"

  metric_transformation {
    name          = local.glue_etl_pythonshell_anomaly_metric_name
    namespace     = local.data_lake_namespace
    value         = "1"
    default_value = "0"
    unit          = "Count"
...
resource "aws_cloudwatch_log_metric_filter" "glue_etl_data_spark_anomaly" {
  name           = "glue-etl-data-spark-anomaly"
  pattern        = local.glue_etl_metric_filter_anomaly_pattern
  log_group_name = "${var.glue_etl_spark_log_group_name}/error"

  metric_transformation {
    name          = local.glue_etl_spark_anomaly_metric_name
    namespace     = local.data_lake_namespace
    value         = "1"
    default_value = "0"
    unit          = "Count"
...
resource "aws_cloudwatch_metric_alarm" "glue_etl_data_anomaly" {
  alarm_name          = "glue-etl-data-anomaly"
  alarm_description   = "Data anomaly logged over 1 minute by a Glue ETL job."
  comparison_operator = "GreaterThanThreshold"
  evaluation_periods  = "1"
  threshold           = "0"
  treat_missing_data  = "notBreaching"
  ...
  metric_query {
    id          = "anomaly_expression"
    expression  = "pythonshell_anomaly + spark_anomaly"
    label       = "Combined Anomalies"
    return_data = true
  }

  metric_query {
    id = "pythonshell_anomaly"
    metric {
      metric_name = local.glue_etl_pythonshell_anomaly_metric_name
      namespace   = local.data_lake_namespace
      period      = 60
      stat        = "Sum"
    }
  ...
  metric_query {
    id = "spark_anomaly"
    metric {
      metric_name = local.glue_etl_spark_anomaly_metric_name
      namespace   = local.data_lake_namespace
      period      = 60
      stat        = "Sum"
    }
